...
describe('GameService', () => {
  ...
  describe('createGame', () => {
    it('creates a new game', async () => {
      jest
        ...
        .mockImplementation(async () => new GameMockFactory().create());
  ...
  describe('joinGame', () => {
    ...
    beforeEach(async () => {
      ...
      player1 = new PlayerMockFactory().create({
        name: 'player-1',
        socketId: 'socket id',
      });
    ...
    it('throws if no name', async () => {
      await expect(
        gameService.joinGame(id, undefined, undefined),
      ).rejects.toThrow(`You must have a name`);
    });
    ...
    it('throws if game at capacity', async () => {
      ...
      await expect(gameService.joinGame(id, 'player-11', '11')).rejects.toThrow(
        `Game is full`,
      );
    });
    ...
    it('throws when new player joins a game in progress', async () => {
      ...
      await expect(gameService.joinGame(id, 'player-2', '20')).rejects.toThrow(
  ...
  describe('leaveGame', () => {
    ...
    it.skip('throws when player leaving not found', async () => {
      await expect(
        gameService.leaveGame(id, 'unkown socket id'),
      ).rejects.toThrow(`This player not found in game ${id}`);
    });

    it('removes a player when game has not started', async () => {
      await gameService.leaveGame(id, 'player-1-socketid');
      const player1StillThere =
        game.players.find((player) => player.name === 'player-1') !== undefined;
