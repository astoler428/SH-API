...
describe('Logic Service', () => {
  ...
  describe('startGame', () => {
    ...
    it('sets hitler knows fasc automatically in 5 or 6', () => {
      ...
      const newGame = new GameMockFactory().create({
        players,
        settings: {
    ...
    it('starts with a red down in redDown setting', () => {
      ...
      game = new GameMockFactory().create({
        players,
        settings: {
          type: GameType.BLIND,
          redDown: true,
    ...
    it('assigns roles properly in 5 player', () => {
      ...
      const hitler: Player[] = game.players.filter(
        (player) => player.role === Role.HITLER,
      );
      const libs: Player[] = game.players.filter(
        (player) => player.team === Team.LIB,
      );
      const fascs: Player[] = game.players.filter(
        (player) => player.team === Team.FASC,
      );
      ...
      for (const player of game.players) {
        expect(
          (player.role === Role.LIB && player.team === Team.LIB) ||
            (player.role === Role.FASC && player.team === Team.FASC) ||
            (player.role === Role.HITLER && player.team === Team.FASC),
        );
      }
    });
    ...
    it('assigns roles properly in 10 player', () => {
      ...
      const allRolesMatch = game.players.every(
        (player) =>
          (player.team === Team.LIB && player.role === Role.LIB) ||
          (player.team === Team.FASC &&
            (player.role === Role.FASC || player.role === Role.HITLER)),
      );
    ...
    it('does not assign lib spy in nonlibspy game', () => {
      ...
      logicService.startGame(game);
      const libSpy = game.players.find(
        (player) => player.role === Role.LIB_SPY,
      );
  ...
  it('assigns roles randomly in mixed game', () => {
    ...
    game = new GameMockFactory().create({
      players,
      settings: { type: GameType.MIXED_ROLES },
    });
    logicService.startGame(game);
    for (const player of game.players) {
      expect(player.role).toBeDefined();
  ...
  describe('determineResultsOfVote', () => {
    ...
    it('passes the vote in an even number of players', () => {
      game.players.push(
        new PlayerMockFactory().create({ name: 'player-6', vote: Vote.JA }),
      );
    ...
    it.skip('does not pass in even number of players', () => {
      game.players.push(
        new PlayerMockFactory().create({ name: 'player-6', vote: Vote.NEIN }),
      );
      expect(game.players).toHaveLength(6);
    ...
    it('it checks hitler condition and makes appropriate calls', () => {
      const mockCheckHitler = jest
        .spyOn(logicService, 'checkHitler')
  ...
  describe('resetVotes', () => {
    beforeEach(() => {
      for (const player of game.players) {
        player.vote = Math.random() - 0.5 < 0 ? Vote.JA : Vote.NEIN;
      }
    });
  ...
  describe('gameOver', () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(logicService, 'topDeckCard')
        .mockImplementation(() => topDeckedCard);
    ...
    it('calls the appropriate functions', () => {
      ...
      expect(logicService.enactPolicy).toBeCalledWith(
        game,
        topDeckedCard,
        true,
      );
  ...
  describe('enactPolicy', () => {
    ...
    beforeEach(() => {
      ...
      jest.clearAllMocks();
      jest
        .spyOn(logicService, 'libSpyCondition')
        .mockImplementation((game) => false);
      player1 = game.players.find((player) => player.name === 'player-1');
      player2 = game.players.find((player) => player.name === 'player-2');
      game.currentChan = 'player-1';
    ...
    it('calls reshuffle if under 3 cards in the deck', () => {
      ...
      game.deck.drawPile = [0, 1, 2].map((i) =>
        new CardMockFactory().createFasc(),
      );
      logicService.enactPolicy(game, libCard, false);
      expect(logicService.reshuffle).not.toBeCalled();
      game.deck.drawPile = [0, 1].map((i) =>
        new CardMockFactory().createFasc(),
      );
      logicService.enactPolicy(game, libCard, false);
  ...
  describe('presClaim', () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(logicService, 'determinePolicyConf')
        .mockImplementation((game) => {});
      jest
        .spyOn(logicService, 'determinePower')
        .mockImplementation((game) => Status.INV);
      jest.spyOn(logicService, 'setPrevLocks');
  ...
  describe('addGov', () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(logicService, 'determineUnderClaim')
        .mockImplementation((game) => 2);
  ...
  describe('determinePower', () => {
    ...
    it('determines no power on lib policy', () => {
      ...
      game.players = [1, 2, 3, 4, 5, 6, 7, 8, 9].map((i) =>
    ...
    it('determines inv in 7 and 8 player games', () => {
      ...
      game.players = [1, 2, 3, 4, 5, 6, 7].map((i) =>
        new PlayerMockFactory().create(),
      );
      expect(logicService.determinePower(game)).toBe(Status.INV);
    });
    ...
    it('no inv in 5 and 6 player games', () => {
      game.chanPlay = new CardMockFactory().createFasc();
      game.FascPoliciesEnacted = 2;
      game.players = [1, 2, 3, 4, 5, 6].map((i) =>
        new PlayerMockFactory().create(),
      );
      expect(logicService.determinePower(game)).not.toBe(Status.INV);
    });
  ...
  describe('shootPlayer', () => {
    ...
    beforeEach(() => {
      const players = [1, 2, 3, 4, 5, 6, 7].map((i) =>
        new PlayerMockFactory().create({ name: `player-${i}` }),
      );
  ...
  describe('vetoReply', () => {
    ...
    beforeEach(() => {
      ...
      jest.spyOn(logicService, 'discard').mockImplementation((card, deck) => {
  ...
  describe('findPlayerInGame', () => {
    it('throws error if no player found', () => {
      expect(() => logicService.findPlayerIngame(game, 'player-DNE')).toThrow(
        `player-DNE is not a player in this game`,
      );
    });
  ...
  describe('Deck', () => {
    ...
    describe('build deck', () => {
      it('properly builds the deck', () => {
        ...
        expect(game.deck.discardPile).toHaveLength(0);
        const fascPolicies = game.deck.drawPile.filter(
          (card) => card.policy === Policy.FASC,
        );
        const libPolicies = game.deck.drawPile.filter(
          (card) => card.policy === Policy.LIB,
        );
